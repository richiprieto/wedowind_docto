...
def main():
    ...
    # Modificar la selección del test_subset para procesar en batches
    test_start = 0
    test_end = len(train_timestamps)  # Usar el tamaño total del dataset
    batch_size = 20
    
    print(f"Procesando dataset completo: {test_end} timestamps en batches de {batch_size}")
    
    # Procesar cada batch de test
    for batch_num, test_start_idx in enumerate(range(test_start, test_end, batch_size)):
        test_end_idx = min(test_start_idx + batch_size, test_end)
        test_subset = train_timestamps[test_start_idx:test_end_idx]
        
        if len(test_subset) == 0:
            break
            
        print(f"\nProcesando batch {batch_num + 1} de test...")
        print(f"Timestamps {test_start_idx} a {test_end_idx}")

        # Cargar el dataset de prueba para este batch
        print("Cargando el dataset de prueba")
        df_test = reader_train.load_all_signals_for_timestamps(dataset_name, test_subset)
        if df_test.empty:
            print(f"Error: No se pudieron cargar datos para el batch {batch_num + 1} de prueba.")
            continue
        ...
        # Modificar los nombres de los archivos de salida para incluir el número de batch
        ...
        ruta_reconstruction_csv = os.path.join('output', f'reconstruction_error_batch_{batch_num + 1}.csv')
        ...
        # Guardar gráfico de detección de anomalías
        ...
        plt.title(f'Detección de anomalías en conjunto de prueba - Batch {batch_num + 1}')
        plt.xlabel('Muestras')
        ...
        plt.tight_layout()
        plt.savefig(f'output/anomaly_detection_batch_{batch_num + 1}.png')
        plt.close()

        print(f"Completado batch {batch_num + 1}")

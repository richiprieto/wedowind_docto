...
sys.path.append("./efficient-kan-master")  # Asegúrate de que la ruta es correcta
...
def generate_sensor_data(sensor_id, num_samples=2000, anomaly=False):
    ...
    if anomaly:
        ...
        anomaly_indices = np.random.choice(
            num_samples, size=int(0.01 * num_samples), replace=False
        )
...
# Definir el autoencoder utilizando KAN
class KANAutoencoder(nn.Module):
    def __init__(self):
        super(KANAutoencoder, self).__init__()
        # Asumiendo que tenemos 3 características de entrada (accel_x, accel_y, accel_z)
        self.encoder = KAN(
            [3, 64, 32, 16, 3]
        )  # Puedes ajustar las dimensiones según sea necesario
        self.decoder = KAN([3, 16, 32, 64, 3])
...
with torch.no_grad():
    for X_batch, y_batch in test_loader:
        encode, _ = model(X_batch)
        embeddings.extend(encode.cpu().numpy())
        targets.extend(y_batch)

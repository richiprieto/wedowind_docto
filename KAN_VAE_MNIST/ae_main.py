...
train_indices = train_set.targets != 0  # Filtrar dígitos que no son '0'
...
class Autoencoder(nn.Module):
    def __init__(self):
        ...
        self.encoder = nn.Sequential(
            ...
            nn.Linear(128, 64),
            nn.ReLU(True),
            nn.Linear(64, 3),  # Capa de cuello de botella
        )
        # Decoder
...
for epoch in range(num_epochs):
    ...
    if (epoch + 1) % 5 == 0:
        ...
        if not os.path.exists("./traditional_autoencoder"):
            os.mkdir("./traditional_autoencoder")
        save_image(pic, "./traditional_autoencoder/image_{}.png".format(epoch + 1))
